...
- 節点厚み感度を `node -> element -> material point -> design variable` に戻す近似感度が、MMA の更新方向として機能する。
- 無次元パラメータ `h0`, `diffusion`, `helmholtz_alpha` の組み合わせが、意図した最大厚みを表現する。
...
`h0` は mm や mesh cell 数ではなく、無次元の厚み上限パラメータとして扱う。初期実装では `thickLSTO.ipynb` と同じ `h0 = 0.1` を使用し、物理単位への変換や mesh cell 数による指定は導入しない。
...
- `current`: `volume` を使い、荷重後の変形に伴う体積変化を投影へ反映する。

既定値は `reference` とする。`current` は変形状態にも依存するため、初期実装では各 MMA 反復で得られた現在体積を厚み制約評価中の固定値として扱う実験的オプションとする。将来、変形依存性まで含む厳密な感度が必要になった場合は、構造解析を介した追加の連成感度を導入する。

### 厚み制約の許容値

数値安定用の許容違反量として `j_max = 0.01` を使用する。
...
2. `thickLSTO.ipynb` から以下をモジュール化して移植する。
   ...
   - 厚み違反評価
   - 随伴 solve
   - Helmholtz sensitivity extension
...
5. `rho_phys` から `G_thick`、厚み場、違反場を可視化する。
6. `G_thick` が直感と合うことを確認してから、`constraint_function` を 2 制約へ拡張する。
7. `max_iter=5` から `10` 程度で、`G_thick` が減少するか確認する。

## Implementation Sketch

`constraint_function` は次のような構成にする。

```python
g_vol, dg_vol = volume_constraint(...)
g_thick, dg_thick = thickness_constraint(...)

constr = jnp.array([g_vol, g_thick])
grad_cons = jnp.vstack([dg_vol, dg_thick])
```
...
`dphi_node/dphi_rho` は正規化 GIMP 投影、`drho_tilde/dx` は既存の `density_filter`、`drho_bar/drho_tilde` は `threshold_filter` の微分を使う。
...
現時点で実装を開始するための未決事項はない。$\delta_\rho=0.05$ の妥当性は、実装後に有限差分感度と最適化挙動で検証する。
